...
\begin{itemize}
\item coronariennes ischémique : RR \female{} > \male.
\item AOMI : RR \times 5, AVC RR \times 2.
\end{itemize}
...
\item sevrage tabac : substituts nicotinique, (bupropion, varénicline en
  dernière ligne),
  ...
  \item activité physique régulière : 3x45min à 75\% $O_2$
    \begin{itemize}
...
\paragraph{Hypertriglycéridémie}
\begin{itemize}
...
\paragraph{Hypolipémiants}
Statines surtout
\begin{itemize}
  \item \dec LDL, \dec TG \inc HDL
 \item ES : myalgies, \inc CPK, \inc transaminases, \inc risque diabète 2
 \item CI : HS, \textit{grossesse}, allaitement
\end{itemize}

\paragraph{Hypercholestérolémies}
...
\begin{itemize}
  ...
  \item Tomoscintigraphie myocardique de perfusion d'effort ou injection
    ...
    \textit{Lorsque valeur prédictive ECG insuffisante}. Coûteux. Éviter si \gls{BBG}
...
ECG : sus-décalage ST $\ge 1$ (frontal) ou $\ge 2$mm ($V_1-V_6$)  sur $\ge 2$ dérivations contiguës. Donne la topographie
...
\begin{itemize}
  ...
  \item insuf mitrale : fuite par prolapsus valvulaire. Ttt chir
\end{itemize}
...
\item Si angioplastie faisable < 120min ou CI à la \gls{FIV} : salle de cathétérisme +
  angioplastie
\item sinon FIV par TNK-tPA. Si échec, angioplastie de sauvetage
...
\item antiagrégants plaquettaires : aspirine + clopidogrel (sauf si angor
  ...
  \item éplérénone : IDM étendu FEVG < 40à%
\end{itemize}
Éventuellement DAI
...
Détresse vitale ?
\begin{itemize}
  \item respi : FR < 10 ou > 30/min, tirage, sueurs, cyanose, $SpO_2$ < 90\%
...
\begin{itemize}
  ...
  \item Douleur aigüe, prolongée, intense, déchirement, irradie dans dos, descend
    vers lombes
  \item Clinique : $\Delta$PAS > 20mmHg (entre 2 bras), abolition 1 pouls, souffle
...
\begin{itemize}
  ...
  \item douleur thoracique augmente inspiration, decubitus. Calmée par
    antéflexion
  \item ECG : sus-ST diffus, sous-PQ, microvoltage
...
\begin{itemize}
  ...
  \item symptomatique : douleur abdo/lombaire $\pm$ choc hémorragique. Risque de
...
\begin{itemize}
  ...
  \item hémoc négative :  amox + acide clavulanique + gentamicine en attendant
\end{itemize}
...
\begin{itemize}
  ...
  \item arc moyen G concave : hypoplasie voie pulmonaire
\end{itemize}
...
Hémibloc : 
\begin{itemize}
  \item antérieur: -30$^{\circ}$, $S_3$ > $S_2$, S en $V_6$
...
\fbox{Toute tachycardie à QRS larges est une tachycardie ventriculaire
...
\begin{itemize}
  ...
  \item \danger{} y penser si : asthme, toux à l'effort, hémoptysie  
...
\begin{itemize}
  \item ascult (attention : effet bouse blanche, HTA masquée, rigidité des
    artères si âgé)
  \item MAPA (ambulatoire 24h, toutes 15min), obj < 130/80 en 24h
  \item automesure à domicile
\end{itemize}
Évaluation :
\begin{itemize}
...
\begin{itemize}
  ...
  \item Sd de Conn : dépisté par kaliémie. \inc aldostérone/rénine. Localisation
    tumeur scanner/IRM\\
    Chir éventuelle
  \item Coarctation aortique (enfant, adulte jeune) : clinique, confirmée IRM.
...
\subsection{Traitement (Si grade 3 ou (grade 1/2 et $\ge 3$FR)}
Objectif : < 140/90
...
\begin{itemize}
  ...
  \item Péricardite récidivante : ttt = colchicine ?
...
\paragraph{Diurétiques de l'anse} Furosémide [Lasilix]
...
Coumadine (ref), fluindione (Previscan)\footnote{N'est plus prescrit en
  initiation depuis 10/2018} = demi-vie longue. Acénocoumarole = demi-vie courte.
...
- effectrice : fixation de l'allergène $\implies$ activation (histamine,
...
1. Gravité ? Suivant abondance, terrain, persistance $\implies$ risque = hématose et asphyxie
...
- trauma $\wedge$ hématocrite pleural/sanguin > 0.5 $\implies$ probable
  hémothorax (urgence \danger)
- amylase pleurale si pancréatique et sous-phrénique
- si triglycéride > 1.1g/L $\implies$ chylothorax
...
- infectieux 
  ...
  - tuberculome ($\implies$ prélèvements)
...
- Supérieur : goître plongeant $\implies$ TMD : continuité glande thyroïde
- Moyen : 
...
** 205 - BPCO\footnote{Bronchopneumopathie chronique obstructive}
\label{sec:205-bpco}
...
1. \textbf{cardiopathie gauche}  : post-capillaire (fréq+++)
1. HTP maladie \textbf{respiratoire chronique}  : pré-capillaire (freq++)
...
- respiratoire : hypoxémie (effet espace mort (non perfusé) $\implies$ effet shunt
...
- dysfonction VD ou élévation biomarqueurs\footnote{BNP, NT-pro-BNP,
...
- \gls{TVS} : \textbf{pas} d'AINS, antiocoagulants en curatif, chirurgie
...
- cyanose (hypoxémie profonde) $\implies$ $O_2$ ASAP
...
1. \textit{Obstruction}  : corps étranger, laryngite, oedème de Quincke, sténose
  trachée, tumeur laryngées
1. \textit{Anomalie radio} Priorité = pneumonie inf, OAP,
...
1. \textit{Radio normale} de novo = EP, PTX compressif, asthme. Si
...
- PTX compressif = urgence $\implies$ aiguille simple + immédiat
