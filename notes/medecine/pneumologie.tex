...
NB : obstruction VAS dûe à {réduction \diameter, \nearrow{} collapsibilité,
...
\subsubsection{BPCO}
TVO lente progressive + inflammation
...
\end{itemize}
\paragraph{CBNC}
Chimiosensible mais rechutes fréquentes et rapides
$
\left \{
  \begin{array}{l}
    \text{chimio si étendue}\\
...
CBPC : médiane de survie = 16-20 si limitée, 8-12 mois si métastatique
\subsection{Cancers secondaires}
Clinique : pauvre, chercher ganglionnaire. (Dypnée, toux, douleur thoracique,
généraux)
...
\item allergique : production IgE spécifiques. Libération de médiateurs par
  mastocytes et bsophiles $\implies$ vasodilatation, bronchoconstriction
\item non-allergique : pas d'exposition préalable, moins sévere 
\end{itemize}
Déf : symptômes CV, respi, cut/digestif mettant en
jeu le pronostic vital immédiatement après contant

\paragraph{Épidémiologie}
35 DC (alimentaire), 40 DC (piqûre) / an
Agents :
\begin{itemize}
\item aliments (90\% enfant) : arachide++, prot. oeuf/lait, fruits exotique
...
\item CV : PAS < 100mmFg (ou -30\%), tachycardie, pâleur, hypotonie, malaise,
  perte conaissance, trouble rythme/conduction, ischémie myocardique
\item respi : de la toux à l'asthme (mortalité $\nearrow$ si asthme mal
  contrôlé)
\item cutanées, muqueures : pruit, sah cutané érythméateux, urticaire et
  sorutout 
  \begin{itemize}
  \item angioedeme : gonflement mal limité ferme, non érythémateux, pas
    prurit
...
\item multivivscérale modérée
\item multivivscérale sévère
\item arrêt respi
\item DC
\end{enumerate}
...
DD : 
\begin{itemize}
\item choc anaphylactique : choc {vagale, septique, cardiogénique,}, hypglycémie, mastocytose
\item Oedeme de Quincke : sd cave sup, érysipèle du visage, angi-eodeme à
  braykinine, inhalation de corps étranger (toujours y penser !!)
\end{itemize}

\subsection{Traitement}
\paragraph{Urgence}
Adrénaline : 0.01mg/kg (< 0.5mg) adulte \faHospitalO{} $\vee$ 0.3mg /5 min jusque
stabilitation en autoinjection 
...
\item allergie hyménoptères avec réaction systémique antérieure (adulte) ou
  plus sévère que cutanéomuq (enfant)
\end{itemize}
...
\item rarement chez l'adulte/ado : trauma facial (dents), bricolage, toruble
...
\begin{itemize}
\item CE mobile (sd de pénétration) : toux quinteuse, suffucation avec tirage,
...
Précision de la figure~\nameref{fig:diag_detresse_respi}
\begin{enumerate}
